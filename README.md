...
6.  **LCEL Chains (Simple, Sequential & Parallel)**: Building composable pipelines using the `|` operator to connect prompts → models → parsers, executing sub-chains concurrently with `RunnableParallel`, and visualizing chain graphs with `get_graph().print_ascii()` and Mermaid diagrams.
