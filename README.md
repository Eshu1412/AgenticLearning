...
### 8. [DOCUMENT_LOADERS/](./DOCUMENT_LOADERS)
This directory covers loading external documents (plain text and PDFs) into LangChain `Document` objects and feeding them into LLM chains for downstream processing.

*   **[text_loaders.py](./DOCUMENT_LOADERS/text_loaders.py)**: Loads a plain-text file (`sample_doc.txt`) using `TextLoader` with UTF-8 encoding. The loaded content is fed into a `RunnableSequence` chain that uses `ChatGoogleGenerativeAI` to generate a 100–120 word summary of the document.
    > **⚠️ Key Learnings & Common Pitfalls:**
...
7.  **Runnable Primitives**: Using explicit `RunnableSequence`, `RunnableParallel`, and `RunnableBranch` classes for fine-grained control over chain composition, mixing multiple model providers, and implementing conditional branching with proper input type mapping.
8.  **Document Loading & Processing**: Loading plain-text and PDF documents using `TextLoader` and `PyPDFLoader`, combining multi-page content, and feeding document data into LLM chains for summarization and structured information extraction.
